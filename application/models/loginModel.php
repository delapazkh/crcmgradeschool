...
class loginModel extends CI_Model {
  ...
  function getTinfo($ses_id){
    $query = $this->db->query('SELECT * FROM teachers WHERE `t_id` =' .$ses_id);
    return $query->row();
  }
  ...
  function getMyWS($glevelid, $subjid){
    //$query = $this->db->query('SELECT * FROM worksheets WHERE `ws_subject` =' .$subjid);
    $this->db->where('ws_g_level', $glevelid);
    $this->db->where('ws_subject', $subjid);
    $query = $this->db->get('worksheets');
    $res = $query->result();
    // foreach ($res as $row) {
    //   $woksh[] = array(
    //       "unit_id" => $row->ws_unit_id,
